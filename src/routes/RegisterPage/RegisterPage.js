...
const RegisterPage = () => {
    const { 
        ...
        formState: { errors }, 
        handleSubmit 
    } = useForm();  // 만약 에러가 난다면 { mode: "onChange" } 넣어주기!!
    const [errorFromSubmit, setErrorFromSubmit] = useState("");
    const [loading, setLoading] = useState(false);  // 프로세스 처리 중 '가입하기' 버튼 중복클릭을 방지하기 위한 상태변수
    const password = useRef();  // 비밀번호와 비밀번호 재입력을 처리하기 위한 현재 password값 가져오기
  ...
  return (
      <div className='auth-wrapper'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='register_container'>
            ...
            <label>비밀번호</label>
            <input className='pass' type='password' placeholder="숫자, 영문, 특수문자 조합 최소 6자"
            ...
            <label>비밀번호 확인</label>
            <input name="password_confirm" type='password' placeholder="비밀번호 재입력"
            ...
            <div className='p1' style={{ color: '#9E9E9E' }}> 이미 계정이 있으신가요? <Link style={{ color: '#38E07D' }} to="/">로그인</Link></div>
        </form>
      </div>
  )
}
